...
8. **Binary XCFramework dependencies require `exportedPackageSettings` and must be wired into Xcode manually.** Source packages (ObjC or Swift) are compiled into `libnativeBridge.a` and linked automatically. Binary targets (XCFrameworks) are not — their symbols will be undefined at Xcode link time. When a product is backed by a binary target, add `exportedPackageSettings { includeProduct = listOf("ProductName") }` to the `swiftPackageConfig` block. After the Gradle build, a local Swift package is generated (the plugin prints its path). Wire it into `project.pbxproj` following the exact structure in [`references/troubleshooting.md`](references/troubleshooting.md) § "Undefined symbol". Key rules for the pbxproj edit:
   - Use `packageReferences` in `PBXProject` — **never** `localPackages` (unrecognised by xcodebuild).
   - The `XCSwiftPackageProductDependency` entry **must** have a `package` field pointing to the local package UUID — omitting it orphans the product and causes `Missing package product '...'`.
   - Always run `xcodebuild -resolvePackageDependencies -project …` **before** `xcodebuild … build`. Building without resolving first will fail with `Missing package product` even when the pbxproj is correct.
