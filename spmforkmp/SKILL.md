...
6. **For every dependency added, setting the minimum OS version is mandatory — not optional.** Read the package-level `.platforms` in its `Package.swift` and set `minIos`/`minMacos`/`minTvos`/`minWatchos` in `swiftPackageConfig` to at least those values for every platform the KMP module targets. Never skip this: a mismatch causes a build error and the defaults (`minIos = "12.0"`, `minMacos = "10.13"`, `minTvos = "12.0"`, `minWatchos = "4.0"`) are often too low for modern packages. If the package has no `.platforms` block, the defaults are safe.
