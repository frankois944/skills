...
6. **For every dependency added, read the package-level `.platforms` declaration in its `Package.swift` and raise `minIos`/`minMacos`/`minTvos`/`minWatchos` in `swiftPackageConfig` to match** — only for the platforms the KMP module actually targets. The defaults (`minIos = "12.0"`, `minMacos = "10.13"`, `minTvos = "12.0"`, `minWatchos = "4.0"`) are often too low for modern packages. A mismatch causes a build error. If the package has no `.platforms` block, the defaults are safe.
